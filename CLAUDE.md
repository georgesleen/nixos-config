...
- `classifierModel` points at a provider the session model does not use, so the gate cannot spend the quota it is protecting. Classifier and title-generator calls never appear in `~/.omp/stats.db`, which records main-agent messages only, yet they bill their own model's plan; cached input also counts fully toward plan rate limits and only the price is discounted.
