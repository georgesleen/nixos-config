...
- `modules/hardware/thinkpad.nix` pcieport wakeup udev rule: the PCIe/Thunderbolt root ports arm wake-from-S4, so the laptop self-wakes from hibernate with the lid shut and sits awake draining flat; only `LID` should wake from S4. Sets `power/wakeup=disabled` on enabled pcieports (idempotent, unlike toggling `/proc/acpi/wakeup`); leaves `XHC` for S3 dock-button wake.
- `hosts/gs-thinkpad-t480s/power.nix` resumeCommands: backstop for any self-wake the pcieport rule misses — the lid switch never fires on a wake-with-lid-closed, so on resume with `LID/state=closed` it re-runs `lidSleepAction` to go straight back to sleep. Can't loop: opening the lid sets state=open and falls through to the network refresh.
