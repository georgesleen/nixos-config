...
- `home/dotfiles/omp.nix` `cacheRetention`, `compaction.idleEnabled`, `classifierModel`, `permissions.allow`: usage tuning from measured session data on 2026-09-11, each reversing an earlier setting. Reasons at each definition. The trap worth repeating here is that `compaction.idleThresholdTokens` defaults to 200000, which is the context ceiling itself, so idle compaction **could never fire at any setting** while it was enabled. Verify any of these in the wrapper's `omp-policy.yml` (`readlink -f $(which omp)`), never with `omp config get`, which cannot see overlay keys.
