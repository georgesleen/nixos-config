...
- `modules/hardware/thinkpad.nix` `dock-ss-recover`: the dock's SuperSpeed bus intermittently comes up dead while the USB2 side enumerates in full, so the RTL8153 (the only USB3-only device on the dock) never appears and only a reboot fixed it. Port reads `state=not attached, disable=0, over_current_count=0`, i.e. a stuck SuperSpeed root port with no connect event to notice; the xHCI runtime-PM hold above does NOT prevent it (2026-08-17 00:50 failed with the hold active). udev starts the oneshot when the dock's USB2 hub (`17ef:30af`) appears; it settles 8s, exits if bus 4 is populated (every plug but two so far), else logs `portsc` from xhci debugfs and escalates: power-cycle just the SuperSpeed root port via `usb4-port1/disable`, and only on failure the full chip reset. A `/run` stamp with a 180s cooldown gives each plug one attempt, since the recovery itself re-plugs the hub and would otherwise retrigger the rule. Do NOT try a bare `xhci_hcd` unbind/rebind: the rebind times out and strands the controller with no USB at all (hit 2026-08-17); only the `force_power` cycle resets Alpine Ridge, which is why `tb-power-cycle` is factored out and shared with `tb-recover`.
