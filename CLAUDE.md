...
- `flake.nix` gs-pi4: uses QEMU binfmt emulation (no `buildPlatform` override). Cross-compilation was attempted but abandoned due to multiple nixpkgs cross-compile bugs: (1) `gh` Makefile CGO_ENABLED issue, (2) .NET SDK `xmlstarlet` in buildInputs not nativeBuildInputs (marksman), (3) Haskell TH external interpreter (`iserv`) crashing for packages with `{-# ANN #-}` annotations (fgl/shellcheck/bash-language-server). If retrying cross-compilation: `buildPlatform` must be hardcoded to `"x86_64-linux"` (not `builtins.currentSystem` — pure eval blocks that builtin).
...
- `modules/desktop.nix` Firefox preferences `media.ffmpeg.vaapi.enabled`/`media.hardware-video-decoding.force-enabled`: Firefox intentionally keeps VA-API opt-in upstream due to historical driver-crash bugs; must be set explicitly even when system VA-API is fully functional.
- `home/dotfiles/i3blocks.nix` gpuBlock: Intel GPU utilization is derived from RC6 residency delta (not a direct utilization counter) — `/sys/class/drm/card*/gt/gt0/rc6_residency_ms` is the only sysfs metric not requiring root or intel-gpu-tools; AMD uses `gpu_busy_percent` directly; NVIDIA falls back to nvidia-smi.
- `home/dotfiles/kanshi.nix` mkMoveScript: `[workspace="N"] move workspace to output X` is not valid in sway IPC (criteria only apply to window containers); workspace migration uses focus+move (`workspace N; move workspace to output X`) with save/restore of the focused workspace to avoid leaving the user on a stale workspace after dock/undock.
- Steam Remote Play client (gs-thinkpad-t480s): white screen with `display ~1000ms` in streaming stats (~1 FPS), both HW and SW decode — the decoder is not the bottleneck. Root cause unknown; the display present/vsync chain stalls under XWayland/sway on the Intel iHD driver. **`LIBGL_DRI3_DISABLE=1` must NOT be used** — XWayland dropped DRI2 support entirely, so disabling DRI3 falls back to llvmpipe software rendering which prevents Steam from launching. Root cause and fix still TBD.
- "open new kitty in last dir" (`Mod3+Shift+Return` in `home/dotfiles/sway.nix` + `mark` function in `home/dotfiles/bashrc.nix`): `kitty --cwd last` does **not** work — `--cwd`/`last` only exist for the kitty `launch` *action* (remote control, inside a running kitty), never the top-level CLI (which only has `-d`/`--working-directory` taking a real path). An invalid flag makes kitty exit non-zero and sway's `exec` silently shows nothing. Since each kitty is its own independent process (no central instance to query), `mark` writes `$PWD` to `$XDG_RUNTIME_DIR/kitty-last-dir` on demand and the keybinding reads it (fallback `$HOME`). `$PWD` must live in a helper function — **not** inline in the `PROMPT_COMMAND="..."` assignment — because Nix `''...''` strings don't interpret `\"` as an escape, so `$PWD` inside a bash double-quoted assignment expands at source time (always `$HOME`). Caveat: stale by one prompt if you `cd` without hitting Enter.
- `hosts/gs-pi4/default.nix` `hardware.enableAllHardware = lib.mkForce false`: `sd-image.nix` imports `profiles/all-hardware.nix` which sets `hardware.enableAllHardware = true`, adding Rockchip/sun4i modules (`dw-hdmi`, `dw-mipi-dsi`, ...) to `boot.initrd.availableKernelModules`. The RPi kernel has none of these; `makeModulesClosure` hard-fails on any listed-but-absent module. `lib.mkForce` overrides the profile's priority-100 default.
