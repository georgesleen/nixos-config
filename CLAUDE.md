...
- `hosts/gs-pi4/default.nix`: **must set `nixpkgs.buildPlatform = builtins.currentSystem`** alongside `hostPlatform = "aarch64-linux"`. Without it, `buildPlatform` defaults to `hostPlatform` and nixpkgs falls back to QEMU binfmt emulation — kernel builds take 24+ hours instead of minutes. `builtins.currentSystem` gives cross-compilation from x86_64 and native builds from the Pi itself.
