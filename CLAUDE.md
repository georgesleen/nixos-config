...
- `modules/features/taildrop.nix` (T480s, photo-into-markdown workflow): the phone shares a photo over the tailnet, a user service drains tailscaled's inbox into `~/Pictures/taildrop`, and Helix's `space i` (`home/dotfiles/cogs/photo-insert.scm`) opens an insert submenu: `i` browses from the directory used last, `d` asks for a directory, `t` browses the inbox, `l` takes the next inbox photo without browsing. `md-photo-pick` chooses the file (yazi in a kitty window by default, overridable with `MD_PHOTO_EXPLORER`, `MD_PHOTO_EXPLORER_CMD` and `MD_PHOTO_TERMINAL`) and remembers its directory in `$XDG_STATE_HOME/md-photo-insert/last-dir`; one prompt then asks for a name, offering the camera's own stem (`IMG_1975`) so Enter alone keeps it, and `md-photo-import` files the photo and inserts `![name](../media/DATE_slug.ext)` at the cursor. The destination is `media/` at the git root, or `photos/` when the repository already has one (the logbook does). Files are **moved** out of the inbox and **copied** from anywhere else, so a picture picked out of a synced directory stays there. `md-photo-pick` runs `tailscale file get` itself on every invocation, so a photo shared seconds ago is on disk before anything is listed; the `--loop` unit stays because photos must also appear when browsing outside Helix. Two flags are load-bearing and both fail quietly. Without `--operator=<user>` on `tailscale set`, the local API answers `Access denied: file access denied` and only root can drain the inbox, so the user service dies on every poll. Without `--loop` on `tailscale file get`, the command takes whatever is waiting and exits, so the first photo arrives and every later one sits in the inbox with the unit shown as `active (exited)`. The picker takes the **oldest** inbox file, not the newest, so photos sent in the order they were taken insert in that order; the importer reads the date from EXIF `DateTimeOriginal` and only falls back to mtime, so a photo filed the next morning still carries the bench date.
...
- `home/dotfiles/omp.nix` `extensionHandlers.toolCallTimeoutMs = 70000`: omp runs its own 30000 ms watchdog over an extension's whole `tool_call` handler, and pi-automode's `autoMode.classifierTimeoutMs` defaulted to the *same* 30000 ms. pi-automode classifies in stages (a fast gate, then a detailed review call, sequentially, each budgeted the full classifier timeout), so any two-stage classification blew omp's outer deadline first. The handler was killed before the extension's own graceful fail-closed path could return a decision, which is why the failure reads as the opaque pair "Auto mode blocked edit: Fast classifier failed" plus `handler timed out after 30000ms` instead of a clean classifier verdict, and why it struck the *large* edits (more context, slower classification). The outer bound now clears two full classifier stages; `classifierTimeoutMs` stays 30000 so a genuinely stuck classifier still fails closed inside it. Note the key lives in omp, not the extension, so `/automode status` shows nothing wrong.
...
- Bang commands put their output in the model's context: `!cmd` is included, `!!cmd` is the local-only form (`isExcluded = text.startsWith("!!")` in the composer submit path, persisted as `excludeFromContext` on the `bashExecution` session entry, and `convertOne` turns exactly the non-excluded ones into a `user` message holding ``Ran `cmd` `` plus the fenced output). Neither spelling is documented upstream; `omp://` names only the message role. What is deferred is the *timing*: `BashRunner.recordBashResult` parks the result in `#pendingMessages` whenever the session is streaming and `flushPending()` appends it only at turn end, so a command run while the agent is working is invisible to that turn and arrives with the next request. Typed interjections steer immediately; bash output has no such path. Verified 2026-09-19: three `!echo hello` runs issued mid-turn produced nothing in that turn and all three landed verbatim at the top of the following one. Inspect any run with `jq -c 'select((.message|type) == "object" and .message.role == "bashExecution") | [.message.command, .message.excludeFromContext]' ~/.omp/agent/sessions/<cwd-slug>/<session>.jsonl`; the `type` guard is not optional, since `jq` on this system is jaq, which errors with "cannot use null as iterable" on `.message.role` for the entries that carry no `message` key at all. No upstream issue or PR covers mid-turn delivery (searched can1357/oh-my-pi 2026-09-19). An extension could do it with a `user_bash` override plus `pi.sendUserMessage(..., { deliverAs: "aside" })`, but the override returns a finished result, so the TUI bash frame never receives chunks and a long command renders blank until it exits, and the parked entry still flushes afterwards, duplicating the output in context. Not worth it while next-turn delivery is enough.
