...
- `hosts/gs-server/win11.xml` QXL+SPICE console: a secondary QXL display + SPICE graphics (`spice://127.0.0.1:5900`) gives virt-manager a console alongside the RX 580 passthrough. Windows makes the QXL display primary, so Sunshine is pinned **in the guest** (`sunshine.conf`: `output_name = {1e786bfd-605a-5292-8dc8-78cb89a85486}`, the RX 580's `\\.\DISPLAY1` / XMD-EDID device) to keep capturing the RX 580 regardless of primary. That `output_name` is guest-side state, not in this repo.
