...
`dock-ss-state`, `gpu-busy`, `lid-decision`, `media-free`, `media-health`,
...
- `modules/hardware/thinkpad.nix` `dock-ss-recover`: the dock's SuperSpeed bus intermittently comes up dead while the USB2 side enumerates in full, so the RTL8153 (the only USB3-only device on the dock) never appears and only a reboot fixed it. Port reads `state=not attached, disable=0, over_current_count=0`, i.e. a stuck SuperSpeed root port with no connect event to notice; the xHCI runtime-PM hold above does NOT prevent it (2026-08-17 00:50 failed with the hold active). udev starts the oneshot when the dock's USB2 hub (`17ef:30af`) appears; it settles 8s, exits if bus 4 is populated (every plug but two so far), else logs `portsc` from xhci debugfs and escalates: power-cycle just the SuperSpeed root port via `usb4-port1/disable`, and only on failure the full chip reset. A `/run` stamp with a 180s cooldown gives each plug one attempt, since the recovery itself re-plugs the hub and would otherwise retrigger the rule. Its `ss_up` guard globbed `4-[0-9]*`, which matches the always-present root hub interface `4-0:1.0`, so the check was always true and the unit exited early every time: the recovery had never once run until this was fixed to `4-[1-9]*` on 2026-08-18 (detection now lives in `modules/hardware/dock-ss-state.sh` with fixture tests run by `nix flake check`). With the guard fixed, real runs on 2026-08-18 recovered the bus for the first time. `Link=RxDetect` with no `CCS` on both SS ports looks like a dead physical link but is NOT one: two host-side attempts (10:48, 10:54) failed against it, then a third at 11:01 succeeded and brought the RTL8153 back. The difference was a dock cold boot (its AC brick and USB-C cable out for 60s) between attempts, which resets the dock's internal VIA USB3.1 hub. Neither half works alone: the host `force_power` cycle cannot revive a wedged dock-side hub, and cold-booting the dock alone did not fix it either. So on an `RxDetect` reading, cold-boot the dock and then let `dock-ss-recover` run, rather than concluding the hardware is faulty. Do NOT try a bare `xhci_hcd` unbind/rebind: the rebind times out and strands the controller with no USB at all (hit 2026-08-17); only the `force_power` cycle resets Alpine Ridge, which is why `tb-power-cycle` is factored out and shared with `tb-recover`.
